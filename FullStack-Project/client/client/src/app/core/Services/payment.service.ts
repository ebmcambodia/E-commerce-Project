...
export class PaymentService {
    ...
    return this.http.post<ResponseDto<any>>(
      ...
      tap(res => {
        if (res.isSuccessed) {
          ...
          this.router.navigate(['/payment/bakong'], {
            state: {
              orderId: res.data.orderId,
              amount: res.data.amountKHR,
              khqr: res.data.khqrString,
              qrImage: res.data.qrImageUrl
            }
          });

        } else {
          this.notification.Error(res.message || 'Unable to create payment');
        }
      })
    );
  }
  ...
  checkPaymentStatus(orderId: string) {
    return this.http.get<ResponseDto<any>>(
      `${this.baseUrl}/Payment/BakongStatus/${orderId}`
    );
  }
}
...
//         if (res.isSuccessed) {
//           const options: any = {
//             key: environment.baseApi, // Enter the Key ID generated from the Dashboard
